...
final class ModernCombatBridge {
    ...
    private static long attacksBlockedUntil;
    ...
    static void onClientTick(Object event) {
        try {
            ...
            if (minecraft == null || player == null || level == null) {
                resetState();
                resetKnockbackDelay();
                ModernHitSelectBridge.onClientTick(null);
                return;
            }
            updateKnockbackDelay(player);
            ModernHitSelectBridge.onClientTick(player);
    ...
    private static void performAttack(Object minecraft, Object player, Object target, boolean allowCriticals) {
        ...
        boolean multiAttack = allowCriticals
                && "Switch".equalsIgnoreCase(ModernForgeEventBridge.mode("KillAura", "Mode", "Single"));
        if (!ModernHitSelectBridge.shouldAttack(player, target, multiAttack)) {
            return;
        }
        AttackMotion attackMotion = captureAttackMotion(player);
        if (allowCriticals) {
            tryCritical(player);
        }
        ...
        ModernHitSelectBridge.onAttack(target);
        if (ModernVelocityBridge.applyAttackSlowdown(player, target,
                attackMotion.x, attackMotion.y, attackMotion.z, attackMotion.available)) {
            // Velocity Attack owns the post-hit slowdown for this attack.
        } else {
    ...
    private static AttackMotion captureAttackMotion(Object player) {
        ...
        if (movement == null) {
            return AttackMotion.UNAVAILABLE;
        }
        return new AttackMotion(
                vectorComponent(movement, "x", "f_82479_"),
                vectorComponent(movement, "y", "f_82480_"),
                vectorComponent(movement, "z", "f_82481_"),
                true);
    }

    private static void applyKeepSprint(Object player, Object target, AttackMotion attackMotion) {
        if (!ModernForgeEventBridge.enabled("KeepSprint") || attackMotion == null) {
            return;
        }
        if (ModernForgeEventBridge.bool("KeepSprint", "Prediction", false) && !attackMotion.available) {
            return;
        }
        if (ModernForgeEventBridge.bool("KeepSprint", "Ground Only", false)
                && !ModernRotationBridge.onGround(player)) {
            return;
        }
        if (ModernForgeEventBridge.bool("KeepSprint", "Reach Only", false)
                && (target == null || Math.sqrt(distanceSq(player, target)) <= 3.0D)) {
            return;
        }
        double slowdown = Math.max(0.0D, Math.min(100.0D,
                ModernForgeEventBridge.number("KeepSprint", "Slowdown", 0.0D)));
        double factor = 0.6D + 0.4D * (1.0D - slowdown / 100.0D);
        setDeltaMovement(player, attackMotion.x * factor, attackMotion.y, attackMotion.z * factor);
    ...
    private static void startUseItem(Object player) {
        String mode = ModernForgeEventBridge.mode("BlockHit", "Mode", "Hypixel");
    ...
    private static void updateKnockbackDelay(Object player) {
        if (!ModernForgeEventBridge.enabled("KnockbackDelay")) {
            resetKnockbackDelay();
            return;
        }
        int hurtTime = intValue(fieldOrInvoke(player, "hurtTime", "f_20916_", "getHurtTime"), 0);
        if (hurtTime <= 0) {
            lastHurtTime = 0;
            return;
        }
        ...
        lastHurtTime = hurtTime;
        if (ModernForgeEventBridge.bool("KnockbackDelay", "Only Weapon", false) && !isHoldingWeapon(player)) {
            return;
        }
        if (ModernForgeEventBridge.bool("KnockbackDelay", "Require Moving", false)
                && !ModernMovementBridge.isMovingForBridge(player)) {
            return;
        }
        if (ModernForgeEventBridge.bool("KnockbackDelay", "Ground Only", false)
                && !ModernRotationBridge.onGround(player)) {
            return;
        }
        double chance = ModernForgeEventBridge.number("KnockbackDelay", "Chance", 100.0D);
        if (RANDOM.nextDouble() * 100.0D > Math.max(0.0D, Math.min(100.0D, chance))) {
            return;
        }
        long base = Math.max(0L, Math.round(ModernForgeEventBridge.number(
                "KnockbackDelay", "Delay MS", 95.0D)));
        long jitter = Math.max(0L, Math.round(ModernForgeEventBridge.number(
                "KnockbackDelay", "Jitter MS", 35.0D)));
        long offset = jitter == 0L ? 0L : Math.round((RANDOM.nextDouble() * 2.0D - 1.0D) * jitter);
        attacksBlockedUntil = System.currentTimeMillis() + Math.max(0L, base + offset);
    }

    private static boolean isAttackDelayed(Object player) {
        return player != null && ModernForgeEventBridge.enabled("KnockbackDelay")
                && System.currentTimeMillis() < attacksBlockedUntil;
    }

    private static void resetKnockbackDelay() {
        lastHurtTime = 0;
        attacksBlockedUntil = 0L;
    }
    ...
    static boolean isProbablyBotForBridge(Object entity) {
        return isProbablyBot(entity);
    }
    ...
    private static final class AttackMotion {
        private static final AttackMotion UNAVAILABLE = new AttackMotion(0.0D, 0.0D, 0.0D, false);
        ...
        private AttackMotion(double x, double y, double z, boolean available) {
            this.x = x;
            this.y = y;
            this.z = z;
