...
final class ModernMovementBridge {
    ...
    private static boolean rightClickDelayUnavailable;
    ...
    static void onClientTick(Object event) {
        try {
            ...
            handleFastPlace(minecraft, player, options);
            handleNoSlow(player);
    ...
    private static void handleFastPlace(Object minecraft, Object player, Object options) {
        if (!ModernForgeEventBridge.enabled("FastPlace") || minecraft == null || player == null || options == null
                || !ModernInputBridge.down(options, "use")) {
            return;
        }
        if (ModernForgeEventBridge.bool("FastPlace", "Only Blocks", true) && !isHoldingBlockItem(player)) {
            return;
        }
        Field field = rightClickDelayField(minecraft);
        if (field == null) {
            return;
        }
        int delay = Math.max(0, Math.min(4,
                (int) Math.round(ModernForgeEventBridge.number("FastPlace", "Delay", 0.0D))));
        try {
            int current = field.getInt(minecraft);
            if (current > delay) {
                field.setInt(minecraft, delay);
            }
        } catch (Throwable throwable) {
            rightClickDelayField = null;
            rightClickDelayUnavailable = true;
        }
    }

    private static boolean isHoldingBlockItem(Object player) {
        Object stack = ModernForgeEventBridge.invoke(player, "getMainHandItem");
        if (stack == null) {
            stack = ModernForgeEventBridge.invoke(player, "m_21205_");
        }
        if (stack == null || Boolean.TRUE.equals(ModernForgeEventBridge.invoke(stack, "isEmpty"))
                || Boolean.TRUE.equals(ModernForgeEventBridge.invoke(stack, "m_41619_"))) {
            return false;
        }
        ...
        Class<?> blockItem = findClass("net.minecraft.world.item.BlockItem");
        return blockItem != null && item != null && blockItem.isInstance(item);
    }

    private static Field rightClickDelayField(Object minecraft) {
        if (rightClickDelayField != null && rightClickDelayField.getDeclaringClass().isInstance(minecraft)) {
            return rightClickDelayField;
        }
        if (rightClickDelayUnavailable || minecraft == null) {
            return null;
        }
        for (String name : new String[]{"rightClickDelay", "rightClickDelayTimer", "f_91011_", "field_71467_ac"}) {
            Field field = findField(minecraft.getClass(), name);
    ...
    static void shutdown() {
        ...
        noSlowTicks = 0;
        longJumpTicks = 0;
        jumpCooldownMisses = 0;
        jumpCooldownField = null;
        rightClickDelayField = null;
        rightClickDelayUnavailable = false;
        diagnosedOnce = false;
        ModernInputBridge.shutdown();
    }
