...
final class ModernPacketBridge {
    ...
    static void shutdown() {
        try {
            ...
            ModernForgeEventBridge.log("Unable to release modern backtrack packets during shutdown", throwable);
        }
        try {
            releaseFakeLagPackets();
        } catch (Throwable throwable) {
            ModernForgeEventBridge.log("Unable to release modern FakeLag packets during shutdown", throwable);
        }
        removeHandler();
        BACKTRACK_PACKETS.clear();
        FAKELAG_PACKETS.clear();
        BACKTRACK_HISTORY.clear();
        BYPASS.clear();
        channelField = null;
        fakeLagNextPulseAt = 0L;
        fakeLagReleaseAfterAttackAt = 0L;
        fakeLagAllowed = false;
    ...
    private static boolean needsNetworkBridge() {
        if (ModernForgeEventBridge.enabled("FakeLag")
                ...
                || ModernVelocityBridge.needsPacketObservation()) {
            return true;
        }
        if (!ModernForgeEventBridge.enabled("Backtrack")) {
            return false;
    ...
    private static boolean onIncoming(Object packet, ChannelHandlerContext ctx) {
        ...
        ModernJumpResetBridge.onIncoming(packet, level, player);
        ModernVelocityBridge.onIncoming(packet);
