...
public class RenderUtil {
    ...
    public static void drawFastRoundedRect(int left, float top, int right, float bottom, float radius, int color) {
        ...
        radius = RenderState.clampRadius(radius, right - left, bottom - top);
        RenderState.begin2D();
        try {
            GL11.glDisable(2884);
            color(color);
    ...
    public static void drawRoundedRect(float x, float y, float x2, float y2, float round, int color) {
        round = RenderState.clampRadius(round, x2 - x, y2 - y);
...
//							R2DUtils.drawRect(x2, y2, x1, y1, color);
    ...
    public static void drawRoundedBorderedRect(float x, float y, float x2, float y2, float radius, float borderWidth, int fillColor, int borderColor) {
        ...
        borderWidth = Math.min(borderWidth, Math.min(Math.abs(width), Math.abs(height)) / 2.0f);
        drawFastRoundedRect(Math.round(x), y, Math.round(x2), y2, radius, borderColor);
        if (x2 - x > borderWidth * 2.0f && y2 - y > borderWidth * 2.0f && getAlpha(fillColor) > 0) {
            drawFastRoundedRect(Math.round(x + borderWidth), y + borderWidth, Math.round(x2 - borderWidth), y2 - borderWidth,
