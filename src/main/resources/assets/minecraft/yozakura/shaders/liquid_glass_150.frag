...
// 1.20.x core-profile version of liquid_glass.frag. The original 1.8.9
// resource stays GLSL 120 for the legacy renderer.
...
void main() {
    ...
    vec2 coord = v_uv * drawSize - vec2(padding);
    ...
    vec3 blurred = texture(screenTex, sourceUv).rgb + noise * u_noise;
    ...
    fragColor = vec4(color * mul, glassAlpha);
}
